...
final class BigDecimalTriMatchWeight<A, B, C> extends AbstractTriMatchWeight<A, B, C> {

    private final TriFunction<A, B, C, BigDecimal> weigher;

    BigDecimalTriMatchWeight(TriFunction<A, B, C, BigDecimal> weigher) {
        this.weigher = Objects.requireNonNull(weigher);
    }

    @Override
    protected UndoScoreImpacter naked(WeightedScoreImpacter<?, ?> impacter, A a, B b, C c) {
        var matchWeight = weigher.apply(a, b, c);
