...
final class BigDecimalBiMatchWeight<A, B> extends AbstractBiMatchWeight<A, B> {

    private final BiFunction<A, B, BigDecimal> weigher;

    BigDecimalBiMatchWeight(BiFunction<A, B, BigDecimal> weigher) {
        this.weigher = Objects.requireNonNull(weigher);
    }

    @Override
    protected UndoScoreImpacter naked(WeightedScoreImpacter<?, ?> impacter, A a, B b) {
        var matchWeight = weigher.apply(a, b);
