...
final class IntUniMatchWeight<A> extends AbstractUniMatchWeight<A> {

    private final ToIntFunction<A> weigher;

    IntUniMatchWeight(ToIntFunction<A> weigher) {
        this.weigher = Objects.requireNonNull(weigher);
    }

    @Override
    protected UndoScoreImpacter naked(WeightedScoreImpacter<?, ?> impacter, A a) {
        var matchWeight = weigher.applyAsInt(a);
