...
public final class ForEachExcludingPinnedUniNode<Solution_, A>
    ...
    private Predicate<A> buildFilter(Solution_ workingSolution, SupplyManager supplyManager) {
        ...
        return entity -> {
            var elementLocation = listVariableStateSupply.getLocationInList(entity);
            if (!(elementLocation instanceof LocationInList elementLocationInList)) {
                return true; // Not in any list, therefore not pinned.
            }
            return !listVariableDescriptor.isElementPinned(workingSolution, elementLocationInList.entity(),
                    elementLocationInList.index());
        };
    }
