...
final class IntTriMatchWeight<A, B, C>
        extends AbstractTriMatchWeight<A, B, C> {

    private final ToIntTriFunction<A, B, C> weigher;

    IntTriMatchWeight(ToIntTriFunction<A, B, C> weigher) {
        this.weigher = Objects.requireNonNull(weigher);
    }

    @Override
    protected UndoScoreImpacter naked(WeightedScoreImpacter<?, ?> impacter, A a, B b, C c) {
        var matchWeight = weigher.applyAsInt(a, b, c);
